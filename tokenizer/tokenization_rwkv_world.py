...
class RWKVWorldTokenizer(PreTrainedTokenizer):
    ...
    def _build_conversation_input_ids(self, conversation: "Conversation") -> List[int]:
        ...
        return input_ids
    def continous_encode(self, text: List[str])->Tuple[torch.Tensor, List[int]]:
        input_ids = []
        for t in text:
            input_ids.append(torch.tensor(self.encode(t, add_special_tokens=False), dtype=torch.int32))
        seq_idx = []
        for i in range(len(input_ids)):
            seq_idx += [i for j in range(input_ids[i].shape[0])]
        input_ids = torch.cat(input_ids, dim=0).unsqueeze(0)
        seq_idx = torch.tensor(seq_idx, dtype=torch.int32).unsqueeze(0)
        return input_ids, seq_idx
